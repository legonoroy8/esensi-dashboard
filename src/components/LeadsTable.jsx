...
export default function LeadsTable({ filters = {} }) {
  ...
  useEffect(() => {
    loadLeads(1); // Reset to page 1 when filters or search change
  }, [filters, searchQuery]);

  const loadLeads = async (page) => {
    try {
      setLoading(true);
      const allFilters = { ...filters, search: searchQuery };
      const data = await api.getRecentLeads(page, pagination.pageSize, allFilters);
      setLeads(data.leads);
    ...
    <div className="bg-white rounded-lg shadow">
      <div className="p-6 border-b border-slate-200">
        ...
        <div className="mt-4">
          <input
            type="text"
            placeholder="Search by name, WhatsApp, interest, or client..."
            value={searchQuery}
          ...
          <tbody className="divide-y divide-slate-200">
            {leads.map((lead, index) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                  {lead.ai_report_sent_at}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                  {lead.sales_rep_replied_at}
                </td>
              </tr>
            ))}
          </tbody>
